  ...
  window.Board = Backbone.Model.extend({
    ...
    initialize: function (params) {
      if (_.isUndefined(params) || _.isNull(params)) {
        ...
        this.set('b', makeBitWiseArray(this.get('n')))
      } else {
    ...
    bitTogglePiece: function(rowIndex, colIndex) {
      var row = this.get(b)[rowIndex];
      var mask = (1 << (this.get('n') - 1));
      mask = mask >> colIndex;
      this.get(n)[rowIndex] = mask ^ row;
    },
 ...
 */
  ...
  var makeBitWiseArray = function(n) {
    return _(_.range(n)).map(function() {
      return 0;
    });
  };
